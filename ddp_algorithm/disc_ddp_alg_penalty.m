function [x, u,J,lambda,dlambda,alpha,k_u,K_u,ii,iter_succ] =disc_ddp_alg_penalty(ddp_2nd_order,f_dyn,run_cost,term_cost,ddp_par,ubar,xbar,opt_par,h);
...
%             V = V_plus + 0.5 * Qu*k_u(:,jj);
            ...
            dJ_exp = dJ_exp + [k_u(:,jj)'*Qu  .5*k_u(:,jj)'*Quu*k_u(:,jj)];
%             V_plus = V;
        end
...
%                 end
                ...
                % penalize the exterior of safe sets
                for kk=1:length(h)
...
%             end   
        ...
        % regularization and cost reduction (Yuichiro) 
...
%             norm_costgrad = dcost;
            if conv_flag
%                 fprintf('convergence of DDP dcost: %.3f\n',dcost);
                break; 
            end
        else
            dlambda   = max(dlambda * lambdaFactor, lambdaFactor);
